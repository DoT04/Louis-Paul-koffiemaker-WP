...
class Walker extends \Walker_Nav_Menu {
            ...
            foreach($desktop_menu_options[$item->ID]['widgets'] as $widget):
                switch(key($widget)):
                    case 'text':
                        ...
                        break;
                    case 'image':
                        $image_alt_text = $this->image_alt_by_url( $widget['image']['url'] );
                        if ( empty( $image_alt_text ) ) {
                            $image_alt_text = "";
                        }
                        $item_output   .= '<img class="responsive-menu-pro-desktop-menu-image" src="' . $widget['image']['url'] . '" alt="'. $image_alt_text .'"/>';
                        break;
                    case 'title':
                        ...
                        $item_output .= '</a>';
                        if ( ! empty( $item->description ) && ! empty( $this->options['submenu_descriptions_on'] ) ) {
                            $item_output .= sprintf('<p class="responsive-menu-pro-item-description"> %s </p>', $item->description );      
                        }
                        break;
                    default: break;
                endswitch;
    ...
    public function start_lvl(&$output, $depth = 0, $args = array()) {
        ...
        $output .= "<ul aria-label='". esc_attr( $this->current_item->title) ."' role='menu' data-depth='" . ($depth + 2) . "' class='" . $extra_class . "responsive-menu-pro-submenu responsive-menu-pro-submenu-depth-" . ($depth + 1) . $this->getSubmenuClassOpenOrNot() . "'>";
     ...
     * @return Bool | String
     */
    private function image_alt_by_url( $image_url ) {
        global $wpdb;
